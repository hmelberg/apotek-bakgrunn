...
Ported from reise/anvil_app/server_modules/utils.py with the log schema
adapted to apotek (no `region` column — we don't filter by category).
"""
...
def authenticate(request) -> Optional[str]:
    """Return the caller's alias if X-API-Key matches a configured key."""
    header_key = None
...
# ---------------------------------------------------------------------------
# Rate limit (per-key per-minute counter)
...
def log_request(
    *,
    ...
    try:
        ...
        # Best-effort — never fail the user request because logging did.
        pass
    return request_id
